...
export class AuthorityMatrix {
  ...
  /**
   * 8. Scout Authority Boundaries:
   * Scouts can source, discover, and submit candidate properties for review.
   * Scouts are STRICTLY FORBIDDEN from verifying, issuing seals, publishing, or making properties bookable.
   */
  static canScoutVerifyOrPublish(user: UserProfile | null): { allowed: boolean; reason: string } {
    if (user?.role === 'scout') {
      return {
        allowed: false,
        reason: 'SECURITY VIOLATION: Scouts have candidate sourcing intake authority only. Scouts cannot verify, seal, publish, or make properties bookable.'
      };
    }
    return { allowed: true, reason: 'Non-scout role' };
  }
}
