...
import { Building2, Eye, CheckCircle2, Calendar, ShieldCheck, Clock, ArrowRight, DollarSign, FileCheck, Layers, AlertCircle, Mail, Sparkles, Send } from 'lucide-react';
import { PropertySupplyStage } from '../types';
import { OwnerMetrics } from '../components/IntelligenceMetrics';
...
export const OwnerView: React.FC<OwnerViewProps> = ({ navigate }) => {
  ...
  const { mode, dataset: { enquiries: roleVisibleRequests, properties, ownerDecisions } } = useOperating();
  const setPropertyRateFloor = (id: string, rate: number) => {
    console.log('Set rate floor', id, rate);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {activeProperty && (
          <>
            ...
            <CalendarSyncSection activeProperty={activeProperty} lang={lang} t={t} />

            {/* Dynamic Communications & Gmail Powerhouse */}
            <div className="my-8 bg-white border border-[#E9DED1] p-6 md:p-8 rounded-sm shadow-xs">
              <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-[#FAF7F2] mb-6 gap-4">
                <div>
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-[#B74C2B]/10 text-[#B74C2B] text-[10px] uppercase font-mono font-bold rounded-xs mb-1">
                    <Mail className="w-3 h-3" />
                    <span>{lang === 'ar' ? 'منظومة Gmail المترابطة' : 'Dynamic Comm Powerhouse'}</span>
                  </div>
                  <h3 className="font-serif-editorial text-2xl text-[#0D2340]">
                    {lang === 'ar' ? 'مراسلات المالك وقرارات التحديث اللحظي' : 'Owner Communications & Live Write-Back Picks'}
                  </h3>
                  <p className="text-xs text-[#6D7480] mt-0.5">
                    {lang === 'ar'
                      ? 'إرسال واستقبال إشعارات Gmail الرسمية، فتح أو حظر الليالي، تقديم استئناف BPS، أو تسويق العقار باللحظات.'
                      : 'Dispatch official emails via your Google Account, declare vacancies, block nights, submit BPS appeals, or launch moment marketing.'}
                  </p>
                </div>
                <button
                  onClick={() => navigate('/communications')}
                  className="px-4 py-2 bg-[#0D2340] hover:bg-[#B74C2B] text-white text-xs font-bold uppercase tracking-wider rounded-xs transition-colors flex items-center gap-1.5 shrink-0 shadow-xs cursor-pointer"
                >
                  <Sparkles className="w-3.5 h-3.5 text-[#C8A15A]" />
                  <span>{lang === 'ar' ? 'فتح مركز المراسلات الكامل' : 'Open Comm Hub'}</span>
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div
                  onClick={() => navigate('/communications')}
                  className="p-4 bg-[#FAF7F2] border border-[#E9DED1] hover:border-[#B74C2B] rounded-xs cursor-pointer transition-all hover:bg-white"
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-[#0D2340]">
                      {lang === 'ar' ? 'إعلان شغور وفتح ليالي' : 'Declare Vacant & Open'}
                    </span>
                    <Calendar className="w-4 h-4 text-[#0F5859]" />
                  </div>
                  <p className="text-[11px] text-[#6D7480] leading-snug">
                    {lang === 'ar' ? 'إخطار المشغلين وفتح ليالي الحجز فورياً بالمنصة.' : 'Notify operators and open reservation calendar in real-time.'}
                  </p>
                </div>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-[#0D2340]">
                      {lang === 'ar' ? 'حظر إقامة المالك' : 'Block Residence Nights'}
                    </span>
                    <Clock className="w-4 h-4 text-[#B74C2B]" />
                  </div>
                  <p className="text-[11px] text-[#6D7480] leading-snug">
                    {lang === 'ar' ? 'حظر التواريخ واستبعادها من الحجوزات مع إشعار المشغل.' : 'Lock owner retreat dates and sync with calendar.'}
                  </p>
                </div>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-[#0D2340]">
                      {lang === 'ar' ? 'استئناف معايير BPS' : 'Send BPS Appeal'}
                    </span>
                    <ShieldCheck className="w-4 h-4 text-[#0D2340]" />
                  </div>
                  <p className="text-[11px] text-[#6D7480] leading-snug">
                    {lang === 'ar' ? 'تقديم اعتراض مدعوم بأدلة لإعادة فحص معايير الجودة.' : 'Formal re-evaluation appeal to quality audit board.'}
                  </p>
                </div>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-[#0D2340]">
                      {lang === 'ar' ? 'تسويق موسع باللحظة' : 'List It & Get Noticed'}
                    </span>
                    <Sparkles className="w-4 h-4 text-[#C8A15A]" />
                  </div>
                  <p className="text-[11px] text-[#6D7480] leading-snug">
                    {lang === 'ar' ? 'بث حملة ترويجية للمسافرين تركز على اللحظة المميزة.' : 'Mass campaign showcase highlighting signature moment.'}
                  </p>
                </div>
              </div>
            </div>
