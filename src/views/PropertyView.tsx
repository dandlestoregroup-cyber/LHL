      ...
      <div className="mt-10 grid gap-12 lg:grid-cols-[1fr_420px]">
        ...
        <aside className="lg:sticky lg:top-40 lg:self-start">
          <div className="rounded-[1.6rem] border border-clay-200 bg-white p-6 shadow-[0_22px_70px_rgba(80,50,35,.09)] md:p-7">
            ...
            {submitted ? (
              <div className="py-10 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-sage-100 text-sage-800"><Check /></div><h3 className="mt-5 font-serif text-2xl text-ink-950">{bi(lang, 'Enquiry created', 'تم إنشاء الطلب')}</h3><p className="mt-2 text-xs leading-6 text-ink-600">{bi(lang, 'The request was accepted by the active data source. Live requests are now server-persisted.', 'تم قبول الطلب من مصدر البيانات الحالي. الطلبات الفعلية تُحفظ الآن على الخادم.')}</p></div>
            ) : (
              <form onSubmit={submit} className="mt-6 space-y-4">
                ...
                <button disabled={submitting} className="button-primary w-full justify-center disabled:opacity-50"><CalendarDays size={15} />{submitting ? bi(lang, 'Creating…', 'جارٍ الإنشاء…') : bi(lang, 'Create stay enquiry', 'إنشاء طلب إقامة')}</button>
              </form>
            )}
          </div>
        </aside>
