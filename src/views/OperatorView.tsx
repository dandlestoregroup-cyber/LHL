              ...
              return <article key={enquiry.id} className="rounded-[1.5rem] border border-clay-200 bg-white p-6">
                <div className="grid gap-6 lg:grid-cols-[1.25fr_.8fr_.8fr_auto] lg:items-center">
                  ...
                  <div className="min-w-[250px] lg:text-end">
                    ...
                    : action ? <button disabled={!canOperate || busy} onClick={() => void runAction(enquiry.id, enquiry.stage)} className="button-primary whitespace-nowrap disabled:cursor-not-allowed disabled:opacity-40">{action}<ArrowRight size={14} className="rtl:rotate-180" /></button> : <StatusPill>{bi(lang, 'No action', 'لا إجراء')}</StatusPill>}
                  </div>
                </div>
