...
export const OperatorView: React.FC<OperatorViewProps> = ({ navigate }) => {
  ...
  const { mode, dataset: { enquiries: roleVisibleRequests, properties }, executeNextEnquiryAction } = useOperating();
  ...
  const handleAction = async (
    ...
    try {
      await executeNextEnquiryAction(reqId, { notes, amount: quoteVal });
      setFeedbackMessage(
        lang === 'ar'
          ? `تم تحديث المرحلة بنجاح`
          : `Request advanced successfully`
      );
    } catch (err: any) {
      setFeedbackMessage(err.message || 'Execution blocked by Authority Matrix');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F2] py-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between pb-8 border-b border-[#E9DED1] gap-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-[#FAF0EB] hover:bg-[#B84E36] text-[#B84E36] hover:text-white border border-[#B84E36]/30 text-xs font-bold rounded-xs transition-all shadow-2xs cursor-pointer"
              title={lang === 'ar' ? 'استوديو صور اللحظات الست' : '6 Canonical Moments Photos Studio'}
            >
              <Camera className="w-3.5 h-3.5" />
              <span>{lang === 'ar' ? 'استوديو صور اللحظات' : 'Moments Photos Studio'}</span>
            </button>
      ...
      {/* Admin Moments Photo Studio Modal */}
