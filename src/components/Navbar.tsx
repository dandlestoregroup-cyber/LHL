...
export const Navbar: React.FC<NavbarProps> = ({ currentPath, navigate }) => {
  ...
  const { mode, setMode } = useOperating();
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className={`${lang === 'ar' ? 'font-arabic-editorial text-xs md:text-sm font-bold' : 'font-brand-script text-base md:text-lg'} text-[#B84E36] leading-none mt-0.5`}>
        ...
        <nav className="hidden lg:flex items-center gap-6 text-xs font-semibold tracking-[0.12em] uppercase text-[#2A201C]">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 transition-colors hover:text-[#B84E36] pb-1 border-b-2 cursor-pointer font-bold ${
              currentPath === '/communications' ? 'border-[#B84E36] text-[#B84E36]' : 'border-transparent text-[#0D2340]'
            }`}
          >
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate('/communications')}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#0D2340] hover:bg-[#1B365D] text-white text-xs font-bold rounded-sm transition-colors cursor-pointer shadow-xs"
            title="Gmail Dynamic Comm Hub"
          >
            <Mail className="w-3.5 h-3.5 text-[#C8A15A]" />
            <span className="hidden sm:inline">{lang === 'ar' ? 'Gmail والتحديثات' : 'Gmail Comm Hub'}</span>
          </button>
